...
void print(Factors*factors){
  ll x=1;
  rep(i,factors->count){
    x*=mypow(factors->list[i].prime_number,factors->list[i].exponent);
  }
  printf("%lld:",x);
  rep(i,factors->count)rep(j,factors->list[i].exponent){
    printf(" %lld",factors->list[i].prime_number);
  }
  puts("");
}
